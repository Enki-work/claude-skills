...
**修正:**
- 該当ファイルをRead・Edit等のツールで修正する
- 修正内容はコメントの指摘に対して必要最小限にとどめる
...
**返信:**
修正内容と対応方針を伝える返信を生成し、投稿する。

返信例:
```
ご指摘の通りです。〜を修正してpushしました。
（修正内容：〜）
```
...
- `owner`, `repo`, `pullNumber`: PR情報から
- `commentId`: ボットコメントのID
- `body`: 返信内容

**一般コメントへの返信（MCP）:**

`mcp__plugin_github_github__add_issue_comment`:
- `owner`, `repo`, `issue_number`: pullNumberと同値
- `body`: 返信内容
